...
| `day03_joins_case_and_strings.sql` | The annotated walkthrough — run it yourself, alongside the reading. |
| `day03_joins_case_and_strings.html` | The same walkthrough, already run — every statement plus its real result set, rendered as a page (generated by `_tools/sql_to_html.py`). No MySQL setup needed; open it to check your own output against, or read through before you have the database loaded. |
