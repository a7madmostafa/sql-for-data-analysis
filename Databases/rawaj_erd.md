...
sellers list products, individual customers buy them. Built — see `rawaj_db.sql` (generated by
`_tools/generate_rawaj_db.py`). See conversation history for the full business description and
naming discussion.
...
`order_items` 11,318 · `web_events` 9,000 · `reviews` 2,792 · `shipments` 9,242. Reproducible —
`_tools/generate_rawaj_db.py` uses a fixed random seed, so re-running produces byte-identical output.

## Open items (not yet decided)

- Whether/how this replaces Parch & Posey and `world` across Days 01–06 (exercise-by-exercise
  migration plan, and rebuilding Day 01 around Rawaj instead of the `world` sample database).
