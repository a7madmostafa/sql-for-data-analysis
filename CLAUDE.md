...
- `Day 03/` — JOINs + CASE, still on `parch_and_posey` (run Day 02's DB script first — no separate setup script here).
  - `D03_01_Materials/day03_reading.md` (bridges Day 01's 1:1/1:N/N:N relationship types to actual JOIN syntax, plus HAVING vs WHERE and CASE) and `Day 03 - JOINs + CASE.pptx` (15 slides, incl. a custom LEFT JOIN row-matching diagram built inline in `build.py` rather than as a new `deckkit` method, since it's not obviously reusable elsewhere).
